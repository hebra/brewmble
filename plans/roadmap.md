...
- [x] **Daemon for macOS**: Expand the daemon to fully support macOS (Homebrew), allowing macOS nodes to be managed just like Linux nodes.
- [x] **Comprehensive Logging**: Implement structured logging (e.g., using `tracing`) in the daemon to facilitate debugging and auditing.
- [x] **Config Management**: Improve CLI configuration handling, including support for multiple profiles and better credential storage.
- [x] **Health Checks**: Add a dedicated health-check endpoint to the daemon for monitoring tools.
